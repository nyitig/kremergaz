...
function startCheckSections() {
  const sectionsIds=document.querySelectorAll('.sectionNav')
  for (let i = 0; i < sectionsIds.length; i++) {
    let sectionIdValue=sectionsIds[i].attributes.id.value
    sectionCoordArr[i]=checkSectionY(sectionIdValue)
    sectionHeightArr[i]=checkSectionBottom(sectionIdValue)
  }
}
...
//  check home
if (rectYCoord>=sectionCoordArr[0]) {
  navLinkBorderErase()
...
if (rectYCoord>=sectionCoordArrModified[1]) {
  ...
  navLinkBorderErase()
    ...
    for (let ind = 0; ind < sectionCoordArr.length; ind++) {
     if (rectYCoord<sectionCoordArr[ind] && rectYCoord>sectionHeightArr[ind]) {
      navLink[ind+1].classList.add('active')
     }      
    }
